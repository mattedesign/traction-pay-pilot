...
const ChatOnlyDashboard = ({
  carrierProfile,
  userProfile
}: ChatOnlyDashboardProps) => {
  ...
  return <div className="flex-1 flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-auto">
        <div className="max-w-4xl mx-auto p-6 space-y-6 px-[8px] py-[12px]">
          {/* Quick Access Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickAccessCards.map((card, index) => <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => handleCardClick(card.topic)}>
      ...
      <div className="border-t bg-white p-4">
        <div className="max-w-4xl mx-auto">
          <ChatInput message={message} onMessageChange={setMessage} onSendMessage={handleSendMessage} isLoading={false} onFocus={handleInputFocus} />
        </div>
      </div>

      {/* Chat Drawer */}
      <ChatDrawer isOpen={isDrawerOpen} onClose={handleCloseDrawer} carrierProfile={carrierProfile} userProfile={userProfile} initialTopic={drawerTopic} initialMessage={drawerMessage} />
    </div>;
};
